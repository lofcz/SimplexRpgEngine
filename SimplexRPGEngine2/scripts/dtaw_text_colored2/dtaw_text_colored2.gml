...
var tmp_input, tmp_sequenceState, tmp_currentColor, tmp_x, tmp_y, tmp_flagString, tmp_baseColor, tmp_startX, tmp_startY, tmp_flagActiveBold, tmp_boldDifY, tmp_flagActiveItalic;
...
for (var i = 1; i <= string_length(tmp_input); i++)
{
	...
	if (tmp_char == "#") {tmp_y += (string_height("#") / 2) + 2; tmp_x = tmp_startX; continue;}
	...
	else if (tmp_char == "]") 
	{
		...
		else
		{
			...
			{			
				...
				draw_sprite(asset_get_index(tmp_flagPart1), real(tmp_flagPart2), tmp_x + 8, tmp_y + 8);
				tmp_x += 16;
	...
	else
	{
		...
		{			
			if (!tmp_flagActiveBold) {fnt(); tmp_boldDifY = 0}
			else {fnt(); tmp_boldDifY = string_height(tmp_char); fnt(fntPixelHuge); tmp_boldDifY = string_height(tmp_char) - tmp_boldDifY;}
			
			clr(tmp_currentColor);
