...
if (v_action == "computeTarget")
{
	...
	{
		if (v_mindState == "idle") {sprite_index = v_weaponOutSprite; v_mindState = "attack"; v_action = "prepareToAttack"; if (v_dir == e_dirs.valD) {image_index = 3.1;} if (v_dir == e_dirs.valA) {image_index = 9.1;} if (v_dir == e_dirs.valW) {image_index = 6.1;} if (v_dir == e_dirs.valS) {image_index = 0.1;}}
		else {v_mindState = "attack";}
	}
	...
	if (v_action == "computeTarget")
	{
		...
		else
		{
			...
			if (v_mindState == "idle")
			{
				...
				if (tmp_choosenDir == "s") {tmp_targetX = x; tmp_targetY = y + 32;}
			}
...
if (v_action == "prepareToAttack")
{

	if (v_dir == e_dirs.valD) {if (image_index < 3 || image_index > 3 * 2 - 0.1) {image_index = 3; v_weaponOut = true; v_action = "computeTarget"; sprite_index = sGoblinMove;}}
	if (v_dir == e_dirs.valA) {if (image_index < 3 * 3 || image_index > 3 * 4 - 0.1) {image_index = 3 * 3; v_weaponOut = true; v_action = "computeTarget"; sprite_index = sGoblinMove;}}
	if (v_dir == e_dirs.valW) {if (image_index < 3 * 2 || image_index > 3 * 3 - 0.1) {image_index = 3 * 2; v_weaponOut = true; v_action = "computeTarget"; sprite_index = sGoblinMove;}}
	if (v_dir == e_dirs.valS) {if (image_index < 0 || image_index > 3 - 0.1) {image_index = 0; v_weaponOut = true; v_action = "computeTarget"; sprite_index = sGoblinMove;}}
}
...
else
{
	if (v_action != "prepareToAttack")
	{
		...
		if (v_action != "attack")
		{
			if (v_weaponOut) {draw_sprite(v_weaponSprite, image_index, x, y);}
			else {draw_sprite(v_weaponIdleSprite, image_index, x, y);}
		}
		else
		{
