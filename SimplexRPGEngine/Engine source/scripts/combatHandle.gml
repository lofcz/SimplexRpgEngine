...
if ((keyboard_check_released(vk_space) && chargeMode) || (keyboard_check_pressed(vk_space) && !chargeMode))
   {   
    if (!attack && apiPlayerGetPropertyValue(vlastnost_stamina, false, true) >= apiPlayerGetPropertyValue(vlastnost_stamina_cost, false, true) && !animating && oPlayer.weaponType != "")
       {
